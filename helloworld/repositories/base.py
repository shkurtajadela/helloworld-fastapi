...
from helloworld.repositories.errors import (
    ForeignKeyViolationError,
    NotFoundError,
    UniqueViolationError,
    RepositoryError,
)
from helloworld.models.base import BaseModel
...
class BaseRepository(Generic[Table, TableCreate]):
    ...
    async def create(
        ...
        statement = insert(self.table).values(entity.dict()).returning(*self.all_columns)
        return await self._process_crud_statement(statement, connection)
    ...
    async def delete(
        ...
        statement = (
            delete(self.table).where(self.table.id == id).returning(*self.all_columns)
        )
        return await self._process_crud_statement(statement, connection)
